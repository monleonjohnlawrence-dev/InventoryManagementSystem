    ...
    public partial class CashierOrder : UserControl
    {
        ...
        private void cashierOrder_removeBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int qty = Convert.ToInt32(dataGridView2.SelectedRows[0].Cells["qty"].Value);
                int prodId = Convert.ToInt32(dataGridView2.SelectedRows[0].Cells["prod_id"].Value);
                    ...
                    @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\InventoryManagementSystem\InventoryManagementSystem\DataBase\inventory.mdf;Integrated Security=True;Connect Timeout=30"))
                {
                    ...
                    // Restore stock because order was removed
                    string restoreStock = "UPDATE products SET stock = stock + @qty WHERE id=@id";
                    SqlCommand cmdRestore = new SqlCommand(restoreStock, connect);
                    cmdRestore.Parameters.AddWithValue("@qty", qty);
                    cmdRestore.Parameters.AddWithValue("@id", prodId);
        ...
        private void cashierOrder_clearBtn_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\InventoryManagementSystem\InventoryManagementSystem\DataBase\inventory.mdf;Integrated Security=True;Connect Timeout=30"))
                {
                    ...
                    string restoreStock = @"UPDATE products SET stock = stock + o.qty
                                    FROM products p INNER JOIN orders o ON p.id=o.prod_id";
                    SqlCommand cmdRestore = new SqlCommand(restoreStock, connect);
                    cmdRestore.ExecuteNonQuery();

                    // Clear orders
                    string deleteOrders = "DELETE FROM orders";
                    SqlCommand cmdDelete = new SqlCommand(deleteOrders, connect);
                    cmdDelete.ExecuteNonQuery();
                }
            ...
            {
                MessageBox.Show("Error clearing orders: " + ex.Message);
            }
        }
        ...
        private void cashierOrder_payOrders_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string customerGuid = Guid.NewGuid().ToString().Substring(0, 8);

                using (SqlConnection connect = new SqlConnection(
                    @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\InventoryManagementSystem\InventoryManagementSystem\DataBase\inventory.mdf;Integrated Security=True;Connect Timeout=30"))
                {
                    connect.Open();
                    SqlTransaction sqlTran = connect.BeginTransaction(); // START TRANSACTION

                    try
                    {
                        // 1️⃣ Get all orders
                        string getOrdersQuery = "SELECT prod_id, prod_name, qty, orig_price, total_price FROM orders";
                        SqlCommand getOrdersCmd = new SqlCommand(getOrdersQuery, connect, sqlTran);
                        SqlDataReader reader = getOrdersCmd.ExecuteReader();

                        List<(int prod_id, string prod_name, int qty, double orig_price, double total_price)> orderItems =
                            new List<(int, string, int, double, double)>();
                        ...
                        while (reader.Read())
                        {
                            orderItems.Add((
                                ...
                                Convert.ToDouble(reader["orig_price"]),
                        ...
                        {
                            MessageBox.Show("No orders to process!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            sqlTran.Rollback();
                            return;
                        }

                        // 2️⃣ Insert customer record
                    ...
                    SELECT CAST(scope_identity() AS int)";
                        ...
                        SqlCommand insertCustomerCmd = new SqlCommand(insertCustomerQuery, connect, sqlTran);
                        insertCustomerCmd.Parameters.AddWithValue("@customer_id", customerGuid);
                        insertCustomerCmd.Parameters.AddWithValue("@total_price", totalAmount);
                        insertCustomerCmd.Parameters.AddWithValue("@amount", amountPaid);
                        insertCustomerCmd.Parameters.AddWithValue("@change_amount", change);
                        insertCustomerCmd.Parameters.AddWithValue("@order_date", DateTime.Now);

                        int insertedCustomerId = (int)insertCustomerCmd.ExecuteScalar();

                        // 3️⃣ Deduct stock & optionally mark product as unavailable if stock = 0
                        foreach (var item in orderItems)
                        {
                            string updateStockQuery = @"UPDATE products 
                                                SET stock = stock - @qty
                                                WHERE id = @id";
                            SqlCommand updateStockCmd = new SqlCommand(updateStockQuery, connect, sqlTran);
                            updateStockCmd.Parameters.AddWithValue("@qty", item.qty);
                            updateStockCmd.Parameters.AddWithValue("@id", item.prod_id);
                            updateStockCmd.ExecuteNonQuery();
                        }

                        // 4️⃣ Insert transactionData record
                        string insertTransactionQuery = @"INSERT INTO transactionData
                    (customer_id, total_amount, cash_paid, change_amount, transaction_date)
                    VALUES (@customer_id, @total_amount, @cash_paid, @change_amount, @transaction_date)";

                        SqlCommand insertTransactionCmd = new SqlCommand(insertTransactionQuery, connect, sqlTran);
                        insertTransactionCmd.Parameters.AddWithValue("@customer_id", insertedCustomerId);
                        insertTransactionCmd.Parameters.AddWithValue("@total_amount", totalAmount);
                        insertTransactionCmd.Parameters.AddWithValue("@cash_paid", amountPaid);
                        insertTransactionCmd.Parameters.AddWithValue("@change_amount", change);
                        insertTransactionCmd.Parameters.AddWithValue("@transaction_date", DateTime.Now);
                        insertTransactionCmd.ExecuteNonQuery();

                        // 5️⃣ Clear orders after successful payment
                        string clearOrdersQuery = "DELETE FROM orders";
                        SqlCommand clearOrdersCmd = new SqlCommand(clearOrdersQuery, connect, sqlTran);
                        clearOrdersCmd.ExecuteNonQuery();

                        sqlTran.Commit(); // ✅ COMMIT transaction
                        ...
                        cashierOrder_receipt_Click(sender, e);
                    }
                    catch (Exception ex)
                    {
                        sqlTran.Rollback(); // ❌ Rollback if any step fails
            ...
            {
                MessageBox.Show("Error in payment process: " + exOuter.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
